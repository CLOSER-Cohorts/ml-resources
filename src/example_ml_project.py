...
#from ml_resources.data import pickle_utility
#from ml_resources.features import create_text_embeddings
...
all_variable_labels={}
#change below so don't have to put square brackets around one study
...
#colectica_utility.get_variables_for_studies(all_studies, all_variable_labels)
...
for study in [all_studies[0]]:
    ...
    #colectica_utility.get_topics_for_items(study_question_items, topics=item_topics)

pickle_utility.save_versioned_pickle_file(all_question_embeddings, 'lha_usoc_question_embeddings', folder='../data')
pickle_utility.save_versioned_pickle_file(item_topics, 'lha__usoc_topics', folder='../data')
...
dataset_lha = create_dataset(identifiersBAK, 
...
X_train, X_test, y_train, y_test = train_test_split(dataset_lha['InputFeatures'],
    dataset_lha['Targets'], train_size=0.7 )    
