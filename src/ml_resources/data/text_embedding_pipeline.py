...
def apply_pipeline(data, columns, training=True):
    ...
    transformed_embeddings = pd.DataFrame({"TextLabel_embeddings": [x[0:embedding_length] for x in transformed_data],
     "ItemCategories_embeddings": [x[embedding_length:embedding_length*2] for x in transformed_data],
     "ItemType": [x[embedding_length*2] for x in transformed_data],
