...
# Here is how we add the original text values for summaries and categories to
# the dataset. We can use these to filter out row where these values have
# certain values (e.g. they are empty, they are "yes/no")
add_input_feature_to_dataset('uk.iser.ukhls',
    identifiers_usoc,
    all_question_summaries,
    "QuestionSummaries",
...
save_versioned_pickle_file(dataset, 'usoc_questions_summaries_and_categories_dataset', folder='../data')
dataset=read_dataset_from_file('../data/usoc_questions_summaries_and_categories_dataset_1.pickle')
dataset_transformed_categories=transform_input_feature_categories(dataset, 'uk.iser.ukhls', 'QuestionCategories')
FINE BUT NOW WITH THE ABOVE YOU NEED TO CALCULATE THE EMBEDDINGS?
YOU NEED TO HAVE A STRATEGY WHERE YOU CAN IMPUTE/CHANGE COLUMNS, DROP ROWS OR COLUMNS,
EG PERHAPS IF GREATER THAN X% ARE BLANK/INVALID, ALL THIS NEEDS TO BE DONE BEFORE YOU
START CALCULATING EMBEDDINGS, SO THIS TRANSFORMATION STEP NEEDS TO BE MOVED UP TO THE TOP
...
X_train, X_test, y_train, y_test = train_test_split(dataset_transformed_categories['uk.iser.ukhls']['InputFeatures'],
    dataset_transformed_categories['uk.iser.ukhls']['Targets'], train_size=0.9 )
X_train[X_train['QuestionCategories'].str.len()==0]